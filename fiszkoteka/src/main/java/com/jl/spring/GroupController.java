...
public class GroupController {
	...
	public String addGroupGet(Model model, @Valid GroupValidator groupValidator, BindingResult result) {
	...
	public String addGroupPost(Model model, @Valid GroupValidator groupValidator, BindingResult result) {
		if(result.hasErrors()){
			model.addAttribute("group", new DBGroup());
			model.addAttribute("message", result.getFieldValue("namegroups"));
	...
	public String listGroupByUser(Model model){
		...
		Integer idUser = (Integer)userService.findUserByEmail(userDetail.getUsername()).getIdUser();
		
		
		List<DBGroup> groups = groupService.findGroupByIdUser(idUser, null, null);
		model.addAttribute("groups", groups);
		if(userDetail.getAuthorities().toArray()[0].toString().equals("ROLE_USER")){
			
			System.out.println(userDetail.getAuthorities().toArray()[0].toString());
	...
	public String enrollToGroup(Model model,  @RequestParam int id){
		...
		ugService.addUserGroup(user, group);
		//model.addAttribute("message", "Zapisa³eœ siê do grupy" + group.getNamegroups());
		
		return "redirect:mylist";
	}
	
	@RequestMapping(value="editname")
	public String editnamePost(Model model, @Valid GroupValidator groupValidator, BindingResult result) {
		if(result.hasErrors()) {
